...
仅仅是给结构体里添加一个数据，就必须定义一个新的结构体。如果我们以后要将这个结构体里添加更多的数据，比如让其储存非整数数据，那么对于每一个需求，我们都需要定义一个新的结构体，并且要将原来结构体里的内容复制过去，这样我们就必须面对很多重复的代码。
...
如果我们的结构体包含更多的数据，比如函数，并且能够一键复制我们之前写过的东西但是不会增加代码量就好了。

当然可以，我们使用面向对象就可以完美解决上面的问题。

我们可以结构体升级，为其赋予新的功能。我们将升级后的结构体叫做**类**（class），**类**除了能保存**数据**外，还能保存**方法**和**特性**。在C\#中，所有的函数都写在类里，没有不在类中的函数，而类中的函数我们叫做**方法**，类中的数据我们叫做**属性**。对于**特性**，我们会在之后的课程中会进行详细探讨。

此外，类除了能保存数据之外的信息之外，还可以将其所有的属性与方法交给其他类，这个过程叫做**派生**。如果一个类派生出了新类，那么我们我们可以说新类**继承**了旧类。派生类具有继承类的所有属性与方法。
...
| public     | 谁都可以访问             |
| internal   | 同一个项目内能访问       |
| protected  | 只有自己和派生类才能访问 |
| private    | 只有我自己能访问         |

后两种访问标识符`protected`和`private`只有在类种的方法、属性和字段才能使用，因此在这里定义类的时候只能使用`public`和`internal`。定义类时的访问标识符可以省去，在这种情况下定义类默认的访问标识符为`internal`，但是为了保证可读性，如果你要写一个`internal`类，那么你就不应该将`internal`省去。
...
但是在不使用`public`字段的情况下，类中的数据永远不会被外界所知，在大部分情况下，这是不符合我们要求的。因此，我们对字段进行**封装**，使用两个方法将其暴露给外部。
...
这样我们就创建了一个名叫`myClass`的`MyClass`实例，`new`与C语言的`malloc`类似，都是申请内存，但是C\#中`new`返回的类型是这个类，`malloc`返回的是`void`指针。而且C\#`new`申请的内存不需要`free`，系统会检测你不需要的内存，自动执行类似C语言`free`的过程。
...
其中手动属性的格式如上所示，并且也可以在`get`或`set`前加上访问限定符限制其访问范围。其中`get`方法是一个没有参数需要返回属性类型的方法，`set`方法是一个有参数返回`void`的方法，其只有一个参数，参数类型与属性类型相同，名称是`value`。此外，手动属性的`get`与`set`是可选的，可以只有一种，也可以两个都有。而自动属性的`get`是必选的，不能只有`set`，没有`get`。知道这些知识，你也可以轻松地编写自己的手动属性。
...
public class OverloadDemo
{
    ...
    public int Overload(long a) //合法，参数数量相同，但具体不同，返回值无所谓相不相同
    {
