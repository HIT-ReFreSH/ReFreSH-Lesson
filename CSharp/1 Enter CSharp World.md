...
此外，C\#中变量可以在代码的任意位置声明，没有C语言C99标准中必须声明在代码块顶端的要求。
...
uint e = a; //不合法，虽然位数相同，有无符号的整型不能相互隐式转换

ulong f = a; //不合法，就转换到的算位数更多，有无符号的整型也不能相互隐式转换
uint g = (uint)a; //合法，有无符号的转换需要使用强制转换

ulong h = d; //合法，无符号的同上
uint i = e; //不合法，无符号的同上

double j = a; //合法，整型能隐式类型转换到浮点类型
